...
def compute_merkle_root_from_proof(proof: MerkleProof):
    ...
    while path:
        ...
        print("compute root from proof, pos: {0}, left: {1}, right: {2}, parent hash: {3}".format(pos, b64encode(left).decode('utf-8'), b64encode(right).decode('utf-8'), b64encode(root).decode('utf-8')))
        pos >>= 1
